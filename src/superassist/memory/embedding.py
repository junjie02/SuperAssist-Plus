...
def _cached_embedder(provider: str, model_name: str, device: str, cache_dir: str) -> Embedder:
    settings = Settings(
        SUPERASSIST_EMBEDDING_PROVIDER=provider,
        SUPERASSIST_EMBEDDING_MODEL=model_name,
        SUPERASSIST_EMBEDDING_DEVICE=device,
        SUPERASSIST_DATA_DIR=str(Path(cache_dir).parent) if cache_dir else ".superassist",
    )
