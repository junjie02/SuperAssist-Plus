...
from superassist.middlewares import SubagentLimitMiddleware
...
from superassist.subagents.store import SubagentResult, SubagentStatus
...
def test_runtime_dispatches_parallel_task_calls_and_receives_results(tmp_path, monkeypatch) -> None:
    task_module = importlib.import_module("superassist.tools.task")
    runtime_module = importlib.import_module("superassist.agent.factory")
    ...
    settings = Settings(
        ...
        SUPERASSIST_SUBAGENT_TIMEOUT_SECONDS=30,
        SUPERASSIST_SUBAGENT_MAX_TURNS=10,
    )
    monkeypatch.setattr(task_module, "get_settings", lambda: settings)
